...
namespace RouterTelnetClient.TelnetClient
{
    ...
    public class Terminal : IDisposable
    {
        ...
        // Telnet commands
        private const byte TNC_SE = 240; // End of subnegotiation parameters

        private const byte TNC_NOP = 241; // No operation

        private const byte TNC_DATAMARK = 242;

        // F2 The data stream portion of a Synch. This should always be accompanied by a TCP Urgent notification. 

        private const byte TNC_BRK = 243; // F3 Break NVT character BRK.

        private const byte TNC_IP = 244; // F4 Interrupt Process The function IP. 

        private const byte TNC_AO = 245; // F5 The function AO. Abort output

        private const byte TNC_AYT = 246; // F6 Are You There The function AYT. 

        private const byte TNC_EC = 247; // F7 Erase character. The function EC. 

        private const byte TNC_EL = 248; // F8 Erase line. The function EL.

        private const byte TNC_GA = 249; // F9 Go ahead The GA signal. 

        private const byte TNC_SB = 250;

        // FA Option code: Indicates that what follows is subnegotiation of the indicated option.

        private const byte TNC_WILL = 251;

        // FB Option code: Indicates the desire to begin performing, or confirmation that you are now performing, the indicated option.

        private const byte TNC_WONT = 252;

        // FC Option code: Indicates the refusal to perform, or continue performing, the indicated option.

        private const byte TNC_DO = 253;

        // FD Option code: Indicates the request that the other party perform, or confirmation that you are expecting the other party to perform, the indicated option.

        private const byte TNC_DONT = 254;

        // FE Option code: Indicates the demand that the other party stop performing, or confirmation that you are no longer expecting the other party to perform, the indicated option.

        private const byte TNC_IAC = 255; // FF Data Byte 255

        // Telnet options
        private const byte TNO_TRANSBIN = 0; // 00 transmit binary

        private const byte TNO_ECHO = 1; // 00 echo

        private const byte TNO_LOGOUT = 18; // 12 Logout

        private const byte TNO_TERMTYPE = 24; // 18 Terminal size

        private const byte TNO_NAWS = 31; // 1F Window size

        private const byte TNO_TERMSPEED = 32; // 20 Terminal speed

        private const byte TNO_REMOTEFLOW = 33; // 21 Remote flow control

        private const byte TNO_XDISPLAY = 35; // 23 X-Display location

        private const byte TNO_NEWENV = 39; // 27 New environment option

        // TELNET others
        private const byte TNX_SEND = 1; // 01 send, e.g. used with SB terminal type

        private const byte TNX_IS = 0; // 00 is, e.g. used with SB terminal type
        ...
        {
            // check for buffer
            if (this.buffer == null) this.buffer = new byte[RECEIVEBUFFERSIZE];

            // virtual screen
            if (this.virtualScreen == null) this.virtualScreen = new VirtualScreen(this.vsWidth, this.vsHeight, 1, 1);

            // set the callbacks
            if (this.callBackReceive == null) this.callBackReceive = new AsyncCallback(this.ReadFromStream);
            if (this.callBackSend == null) this.callBackSend = new AsyncCallback(this.WriteToStream);
            ...
            if (this.tcpClient != null) return true; // we still have a connection -> ?? better reconnect ??
            else
            {
                try
                {
        ...
        public bool SendResponse(string response, bool endLine)
        {
            try
            {
                ...
                byte[] sendBuffer = (endLine)
                                        ? System.Text.Encoding.ASCII.GetBytes(response + ENDOFLINE)
                                        : System.Text.Encoding.ASCII.GetBytes(response);
                if (sendBuffer == null || sendBuffer.Length < 1) return false;
        ...
        {
            if (this.virtualScreen == null || searchFor == null || searchFor.Length < 1) return null;
        ...
        {
            if (this.virtualScreen == null || regEx == null || regEx.Length < 1) return null;
            int sleepTimeMs = this.GetWaitSleepTimeMs(timeoutSeconds);
            DateTime endTime = this.TimeoutAbsoluteTime(timeoutSeconds);
            string found = null;
            do // at least once
        ...
        {
            // 1st check
            if (this.virtualScreen == null || timeoutSeconds < 0) return false;

            // reset flag: This has been added after the feedback of Mark
            if (this.virtualScreen.ChangedScreen) this.virtualScreen.Hardcopy(false);
            ...
            do // run at least once
            {
                lock (this.virtualScreen)
                {
                    if (this.virtualScreen.ChangedScreen) return true;
                }
                Thread.Sleep(sleepTimeMs);
            }
            while (DateTime.Now <= endTime);
        ...
        {
            if (timeoutSeconds < 0 && this.timeoutReceive < 0) return 0;
            else if (timeoutSeconds < 0) return this.timeoutReceive; // no valid timeout, return other one
            else return (timeoutSeconds >= this.timeoutReceive) ? timeoutSeconds : this.timeoutReceive;
        }
        ...
        {
            if (asyncResult == null || this.tcpClient == null) this.Close();
            ...
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    if (this.forceLogout) this.Close();
                    else
                        this.tcpClient.GetStream()
                            .BeginRead(this.buffer, 0, this.buffer.Length, this.callBackReceive, null);
                }
                else
        ...
        {
            // reponse to server
            MemoryStream response = new MemoryStream(SENDBUFFERSIZE);
            // answer usually will be small: "a few bytes only"
                            ...
                            switch (this.buffer[bc])
                            {
                                case TNC_DO:
                                    ...
                                    switch (this.buffer[bc])
                                    {
                                        ...
                                        case TNO_TERMSPEED:
                                            ...
                                            break;
                                        case TNO_NAWS:
                                            if (!this.clientInitNaws) Terminal.TelnetWill(TNO_NAWS, response);
                                            // negotiation about window size
                                            Terminal.TelnetSubNAWS(
                                            ...
                                            break;
                                        case TNO_TERMTYPE:
                                            Terminal.TelnetWill(TNO_TERMTYPE, response);
                                            // negotiation about terminal type
                                            break;
                                        case TNO_XDISPLAY:
                                            Terminal.TelnetWont(TNO_XDISPLAY, response);
                                            // no negotiation about X-Display
                                            break;
                                        case TNO_NEWENV:
                                ...
                                case TNC_SB:
                                    ...
                                    switch (this.buffer[bc])
                                    {
                                        ...
                                        case TNO_TERMTYPE:
                                            bc++;
                                            // the follwing byte is usually a send-request ("SEND"), just read the byte 
                                        ...
                                        default:
                                            ...
                                            while (this.buffer[bc] != TNC_SE && bc < this.buffer.Length)
                                            {
                                                bc++;
                                            }
                                            break;
        ...
        private int ParseEscSequence(int bc, MemoryStream response)
        {
            ...
            if (this.buffer == null) return bc;

            // find the byte after ESC
            if (this.buffer[bc] == ESC) bc++;
            ...
            else if ((m = this.MatchSequence(bc, "M")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "7")) > -1) return (bc + m - 1);
            ...
            else if ((m = this.MatchSequence(bc, "#3")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "#4")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "#5")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "#6")) > -1) return (bc + m - 1);
                // Erasing
            ...
            else if ((m = this.MatchSequence(bc, "1")) > -1) return (bc + m - 1);
            ...
            else if ((m = this.MatchSequence(bc, "H")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[g")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[0g")) > -1) return (bc + m - 1);
            ...
            else if ((m = this.MatchSequence(bc, "[?1h")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?1l")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?3h")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?3l")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?4h")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?4l")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?5h")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?5l")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?6h")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?6l")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?7h")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?7l")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?8h")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?8l")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?9h")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "[?9l")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "1")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "2")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "=")) > -1) return (bc + m - 1);
            ...
            else if ((m = this.MatchSequence(bc, "F")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "G")) > -1) return (bc + m - 1);
            ...
            else if ((m = this.MatchSequence(bc, "I")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "J")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "K")) > -1)
            {
                this.virtualScreen.CleanLine(this.virtualScreen.CursorX, this.virtualScreen.CursorXRight);
                return (bc + m - 1);
            }
            else if ((m = this.MatchSequence(bc, "Ylc")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "Z")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "=")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, "<")) > -1) return (bc + m - 1);
            else if ((m = this.MatchSequence(bc, ">")) > -1) return (bc + m - 1);
            // nothing found
        ...
        private int MatchSequence(int bufferCounter, byte[] sequence, int[] ignoreIndex)
        {
            if (sequence == null || this.buffer == null) return -1;
            if (this.buffer.Length < (bufferCounter + sequence.Length)) return -1; // overflow
            for (int i = 0; i < sequence.Length; i++)
            {
                if (this.buffer[bufferCounter + i] != sequence[i])
                {
                    // not a match
                    if (ignoreIndex == null || ignoreIndex.Length < 1) return -1; // no wildcards
                    bool wildcard = false;
                    ...
                    if (!wildcard) return -1; // no wildcard found
                } // no match
            }
        ...
        {
            if (regExp == null || regExp.Length < 1) return -1;
            else return this.MatchRegExp(bufferCounter, new Regex(regExp));
        }
        ...
        {
            if (r == null || this.buffer == null || this.buffer.Length < bufferCounter) return -1;
            // build a dummy string
            // which can be checked
            string dummyString = null;
            const int dsl = 10;
            if (this.buffer.Length >= (bufferCounter + dsl)) dummyString = Encoding.ASCII.GetString(this.buffer, bufferCounter, dsl);
            else dummyString = Encoding.ASCII.GetString(this.buffer, bufferCounter, this.buffer.Length - bufferCounter);
            if (dummyString == null || dummyString.Length < 1) return -1;
        ...
        private static int NewCursorXPosition(string escSequence)
        {
            ...
            if (escSequence == null) return -1; // error
        ...
        private static byte[] MemoryStreamToByte(MemoryStream ms)
        {
            ...
            else if (ms.Position < 2) return new byte[0];
        ...
        public static string FindIPAddress(string candidate)
        {
            ...
            if (m != null && m.Success) return m.Value;
            else return null;
        }
